...
public class HomeWork7 {
    ...
    public static List<String> methodOne(List<String> names) {
        return names.stream().filter(n->n.contains("King")).toList();
    }
    ...
    public static List<String> methodTwo(List<String> names) {
        return names.stream().map(n->n.split(" ")[1]).distinct().toList();
    }
    ...
    public static List<String> methodThree(List<String> names) {
        // Твой код здесь
        return names.stream().map(n->n.split(" ")[0]).distinct().filter(n->n.startsWith("L")).toList();
    }
    ...
    public static List<String> methodFour(List<String> names) {
        // Твой код здесь
        return names.stream().map(n->n.split(" ")[1]).distinct().filter(n->n.startsWith("W")).sorted().toList();
    }
    ...
    public static Integer methodFive(List<String> names) {
        // Твой код здесь
        return (int) names.stream().map(n->n.split(" ")[0]).distinct().filter(n->n.length()>6).count();
    }
    ...
    public static Integer methodSix(List<String> names) {
        // Твой код здесь
        return (int) names.stream().map(n-> Arrays.asList(n.split(" "))).flatMap(Collection::stream).filter(s->s.length()>5).distinct().count();
    }
    ...
    public static List<String> methodSeven(List<String> names) {
        // Твой код здесь
        return names.stream().map(n->n.split(" ")).filter(arr->arr[1].startsWith("K") || arr[1].startsWith("S")).map(arr->arr[0]+" "+arr[1].charAt(0)+".").distinct().sorted().toList();
    }
